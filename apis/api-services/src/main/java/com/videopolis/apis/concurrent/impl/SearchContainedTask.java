...
/**
 * An {@link ApisTask} which invokes a contained search action and a CAL item
 * fetch of the returned elements.
 * 
 * @author Christophe Fondacci
 * 
 */
public class SearchContainedTask extends AbstractSearchApisTask {

    private final ItemKey parentKey;

    /**
     * Creates a new {@link SearchContainedTask} with the specified information
     * 
     * @param service
     ...
     */
    public SearchContainedTask(final CalService service,
	    final ApisContext context, final String requestedType,
	    final ItemKey parentKey, final PaginationSettings pagination) {
	...
	final SearchResponse response = getContext().getSearchService()
		.searchIn(parentKey, settings, window);
	...
	return "searchContainedTask;" + super.toString() + ";" + parentKey;
    }
}
