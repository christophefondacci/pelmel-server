...
public class AdminAction extends AbstractAction implements MediaAware,
	...
	protected String doExecute() throws Exception {
		...
		if (filterOnline != null && !filterOnline.isEmpty()) {
			filters.add(Boolean.valueOf(filterOnline) ? FilterField.ONLINE
					: FilterField.OFFLINE);
		}

		if (filterIndexed != null && !filterIndexed.isEmpty()) {
			filters.add(Boolean.valueOf(filterIndexed) ? FilterField.INDEXED
					: FilterField.UNINDEXED);
		}

		final RequestTypeListPlaces requestType = new RequestTypeListPlaces(
				pageSize, page, requestSortField, sortAsc, filters);

		final ApisRequest request = (ApisRequest) ApisFactory
				.createCompositeRequest()
				.addCriterion(
						(ApisCriterion) SearchRestriction
								.list(Place.class, requestType)
		...
		searchSupport.initialize(SearchType.BARS, getUrlService(), getLocale(),
				null, null, null, null, places);
